...
impl Display for Explanation {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let res = match self {
            ...
            Explanation::SingleSyllableWithAccent => Cow::Borrowed(
        ...
        match syllable_pos {
            SyllablePosition::Ultima => {
                ...
                if word_pos.ty.is_verb()
                    && (word_pos.normalized_word.ends_with('α')
                        || word_pos.normalized_word.ends_with("αν"))
                {
...
pub fn polytonize_word(
    ...
    if ctx.previous_was_double_accented {
        ...
        return PolytonizedWord::with_explanation(
            res_word,
            Explanation::CliticFollowedByDoubleAccent,
        );
    }
    ...
    'outer: {
        ...
        if accent_info.is_none() {
            ...
            } else if !KNOWN_NON_ACCENTED_SINGLE_SYLLABLE
                .contains(word_pos.normalized_word.as_str())
            {
                inverted_accent_pos = Some(0);
                add_accent(&mut res_word, &syllables);
            ...
            if inv_acc_pos == 2 {
                ...
                should_circum =
                    should_circum_penult(word, word_pos, ult, penult, &mut res_explanation);
            } else {
    ...
    if !should_circum {
        if res_word.to_uppercase() != word
            && inverted_accent_pos.is_some_and(|position| position == 0)
        {
            ...
            if !ALWAYS_ACUTE.contains(word_pos.normalized_word.as_str())
                && !next_is_punct
                && add_grave
            {
