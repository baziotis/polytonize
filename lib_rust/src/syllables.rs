...
impl DoubleEndedIterator for GreekSyllablesIter<'_> {
  fn next_back(&mut self) -> Option<Self::Item> {
    if self.tail >= self.head as isize {
      let res = &self.syllables.word[self.syllables.syllable_ranges[self.tail as usize].clone()];
      self.tail -= 1;
      Some(res)
    } else {
      None
...
impl<'s> GreekSyllables<'s> {
  pub fn new(word: &'s str, norm_word: &str) -> Self {
    ...
    while let Some(norm_c) = norm_iter.next() {
      ...
      let next_word_i = if next_norm_c.is_greek_vowel() && [norm_c, next_norm_c].is_diphthong() {
        norm_iter.next();
        word_iter.next();
